...
export const StatusBar = ({ label, hidden, onShow, onHide, onExit, onLogout }) => {
  ...
  useEffect(() => {
    ...
    const load = () =>
      api.live(controller.signal).then((d) => setLive(d.items.length)).catch(() => {});
    load();
  ...
  return (
    <div className="hm-root fixed inset-x-0 bottom-0 z-[390] flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-primary/40 bg-[#020a04]/95 px-4 py-2 font-mono text-[11px] text-[#7dff7d] backdrop-blur sm:text-xs">
      ...
      <span className="opacity-70">{label ?? "operator"}</span>
