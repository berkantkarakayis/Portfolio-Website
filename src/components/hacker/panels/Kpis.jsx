...
const Tile = ({ label, value, sub, accent = false }) => (
  <div className="hm-panel min-w-0 p-3 sm:p-4">
    <p className="truncate text-[9px] font-bold uppercase tracking-[0.18em] text-[color:var(--muted-color)] sm:text-[10px]">
      {label}
    </p>
    <p className={`mt-1.5 truncate text-xl font-bold leading-none tabular-nums sm:mt-2 sm:text-2xl lg:text-3xl ${accent ? "text-primary" : "text-[#b6ffb6]"}`}>
      {value}
    </p>
    {sub && <p className="mt-1.5 truncate text-[10px] text-[color:var(--muted-color)] sm:text-[11px]" title={sub}>{sub}</p>}
  </div>
);
